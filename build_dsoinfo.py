...
# build says which. M1, M45, M76 and M97 used to be in that position and are
# not any more: build_deepsky.py now admits anything with a Messier number
# whatever its recorded magnitude, and hand-adds the Pleiades, which have no
# NGC number to be found under.
...
def main():
    ...
    # deepsky.json labels an object "M31" in its n field. That field used to
    # be ambiguous -- NGC205 also carried "M31" and NGC595 also carried "M33"
    # -- so this took the brightest match to land on the real ones. Fixed at
    # the source in build_deepsky.py, which now reads the Messier number from
    # RNGC's cross-reference column instead of its prose. The brightest-match
    # tie-break stays as a cheap guard rather than an assumption that the
